...
public class DmlOperations {
   ...
   */
  public int insert(QueryDefinition query,
                    @ParameterGroup StatementAttributes statementAttributes,
                    @ParameterGroup AutoGeneratedKeyAttributes autoGeneratedKeyAttributes,
                    @UseConfig DbConnector connector,
                    @Connection DbConnection connection) throws SQLException {

    QueryStatementFactory statementFactory =
        getStatementFactory(statementAttributes, false, query.getSettings());

    final Query resolvedQuery = queryResolver.resolve(query, connector, connection);
    validateQueryType(resolvedQuery.getQueryTemplate(), asList(INSERT, STORE_PROCEDURE_CALL));

    return (int) new UpdateExecutor(statementFactory)
        .execute(connection, resolvedQuery, getAutoGeneratedKeysStrategy(autoGeneratedKeyAttributes));
  }

  public int bulkInsert(QueryDefinition query,
                        @ParameterGroup StatementAttributes statementAttributes,
                        @ParameterGroup AutoGeneratedKeyAttributes autoGeneratedKeyAttributes,
                        @UseConfig DbConnector connector,
                        @Connection DbConnection connection) throws SQLException {
